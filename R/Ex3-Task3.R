# If the previous task has been completed then dataRead, dataWrite
...
head(airlineCsv)
# Or Microsoft Machine Learning Server functions 
...
system.time(rxImport(inData = airlineCsv, outFile = airlineXdf,
    overwrite = TRUE))
...
# NOTE that the XDF file is giving us approx. 5x saving in space and I/O
# -------------------------------------------------------------------------
...
# Get some information about the columns and data in the xdf 
rxGetVarInfo(data = airlineXdf)
# Note: Microsoft Machine Learning Server pre-computes and stored the low/high values for numeric data 
# which can be useful in optimizing other processing. Categorical data 
# is stored as integer "levels" with metadata referencing the factor  
# values. Microsoft Machine Learning Server provides methods for the
# following functions for supported # data-sources 
head(airlineXdf)
tail(airlineXdf)
names(airlineXdf)
dim(airlineXdf)
...
# Note: Microsoft Machine Learning Server uses R's formula interface to enable a subset of columns 
# to be analysed. 
# For all numeric and factor columns you can use 
rxSummary(~., airlineXdf, reportProgress = 1)
# or Microsoft Machine Learning Server provides a method for summary that does the same thing 
summary(airlineXdf)
...
df <- rxCor(formula = ~DepDelay + ArrDelay + AirSpeed,
                data = airlineXdf,
...
system.time({
    airline.model <- rxLinMod(formula = AirSpeed ~ DepDelay,
                data = airlineXdf,
                rowSelection = (AirSpeed > 50) & (AirSpeed < 800),
                reportProgress = 1)
})
...
# -------------------------------------------------------------------------
# Cleanup
if (file.exists(file.path(dataWrite, "2007.xdf"))) file.remove(file.path(dataWrite, "2007.xdf"))